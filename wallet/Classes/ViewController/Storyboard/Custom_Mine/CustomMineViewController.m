...
- (void)viewDidLoad {
    ...
    self.walletName.text = @"CCM Wallet";
...
-(void)getETHNoticeTransRecord{
    ...
    if (![obj isEqualToString:@""]&&[obj length] > 0) {
        [CFQCommonServer getAllETHNoticeTransRecordPage:1 address:obj complete:^(NSMutableArray<CustomNotificationCenterItemModel *> * _Nonnull dataArray, NSString * _Nonnull errMsg) {
            [SKUtils endLoadMoreForScrollView:self.mainTableView];
            if (errMsg) {
                [MBProgressHUD showMessage:errMsg];
            } else {
                if (dataArray.count <20) {
                    [self getTokenNoticeTransRecordPage:dataArray.count];
