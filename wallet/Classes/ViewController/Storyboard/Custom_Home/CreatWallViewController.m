...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.row) {
        case 0:
            {//jinlai进来默认创建了ccm钱包
                [MBProgressHUD showMessage:@"最多只能存在一个CCM地址！"];
                //ccm  程序进来.默认创建了当前钱包
...
//                }];
        ...
        case 1:
        {
            ...
            if (ishaveBtc) {
                [MBProgressHUD showMessage:@"最多只能存在一个BTC地址!"];
            } else {
                //开始创建钱包
                CreatWallWithTypeView *importWallet = [CreatWallWithTypeView initViewWithXibIndex:0];
                @weakify(self);
...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath{
    ...
    NSArray *BgImageNameARR = [NSArray arrayWithObjects:@"CCM",@"BTC",@"ETH", nil];
    NSArray *IconImageNameARR = [NSArray arrayWithObjects:@"logo_ccm",@"logo_btc",@"logo_eth", nil];
    NSArray *TitleARR = [NSArray arrayWithObjects:@"CCM-Wallet",@"BTC-Wallet",@"ETH-Wallet", nil];
