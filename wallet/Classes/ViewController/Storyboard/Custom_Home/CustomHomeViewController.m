    ...
    switch (subIndex) {
        ...
        case 1:
        {
            ...
            if (ishaveBTC) {
                ...
                [MBProgressHUD showMessage:@"请先创建BTC钱包"];
            }
        }
            break;
        case 2:
        {
            ...
            if (ishaveZWETH) {
               ...
               [MBProgressHUD showMessage:@"请先创建ETH钱包"];
            }
        }
            break;
...
//                    [[NSUserDefaults standardUserDefaults] synchronize];
                ...
                [CSingleSample instance].sum += item.balance * item.exchange;
...
- (void)reloadWallet:(NSMutableArray *)array {
    ...
    for (QYHangQingModel * ExchangeModel in array) {
        if (ExchangeModel && [ExchangeModel isKindOfClass:[QYHangQingModel class]]) {
            id coinName = ExchangeModel.lname;
            double rmgExchageRate = [ExchangeModel.latest_price_xs doubleValue];
            if (coinName && [coinName isKindOfClass:[NSString class]]) {
                if (self.model.dataArray.count != 0 && self.model.dataArray.count == 26) {
                    CFQtestSwift *textSwift = [[CFQtestSwift alloc]init];
    ...
    //回调或者说是通知主线程刷新，
    [MBProgressHUD showMessage:@"资产数据处理成功"];
...
- (void)showWallet {
    if (CurrrentWallIndex == 0) {
            ...
            switch (type) {
                ...
                case CustomImportWalletType_MnmonicWord:
                {
                    ...
                    if ([mnmonic isEqualToString:kMNullStr]) {
                        [MBProgressHUD showMessage:@"当前地址不存在可以导出的助记词。请确认导入方式是否为助记词或者是否为新建钱包"];
                        return;
                    }
        ...
        if (ishaveBTC) {
            ...
            [MBProgressHUD showMessage:@"请先创建钱包"];
        }
        ...
        if (ishaveZWETH) {
            ...
            [MBProgressHUD showMessage:@"请先创建以太坊钱包"];
        }
