//
//  QYHangQingModel.h
//  wallet
//
//  Created by talking　 on 2019/7/4.
//  Copyright © 2019 talking　. All rights reserved.
//

#import "SKBaseModel.h"

NS_ASSUME_NONNULL_BEGIN

@interface QYHangQingModel : SKBaseModel
//@property (nonatomic, copy) NSString *change_percent;
//@property (nonatomic, copy) NSString *code;
//@property (nonatomic, copy) NSString *current_price_usd;
//@property (nonatomic, copy) NSString *fullname;
//@property (nonatomic, copy) NSString *logo;
//@property (nonatomic, copy) NSString *market_value_usd;
//@property (nonatomic, copy) NSString *marketcap;
//@property (nonatomic, copy) NSString *name;
//@property (nonatomic, copy) NSString *star_level;
//@property (nonatomic, copy) NSString *supply;
//@property (nonatomic, copy) NSString *vol_usd;




@property (nonatomic, copy) NSString *xid;
@property (nonatomic, copy) NSString *lname;
@property (nonatomic, copy) NSString *fname;
@property (nonatomic, copy) NSString *volume;
@property (nonatomic, copy) NSString *latest_price;
@property (nonatomic, copy) NSString *up_down;
@property (nonatomic, copy) NSString *is_lock;
@property (nonatomic, copy) NSString *latest_price_x;
@property (nonatomic, copy) NSString *latest_price_xs;
//@property (nonatomic, copy) NSString *supply;
//@property (nonatomic, copy) NSString *vol_usd;
@end

NS_ASSUME_NONNULL_END
