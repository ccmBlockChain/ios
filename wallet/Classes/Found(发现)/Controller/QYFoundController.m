...
-(UITextField *)TexFD{
    if (_TexFD == nil) {
        ...
        _TexFD.placeholder = @"搜索或输入DApp网址";
...
- (NSMutableArray *)dataArray {
    if (!_dataArray) {
        _dataArray = [[NSMutableArray alloc]initWithObjects:@{@"icon":@"babite",@"name":@"巴比特论坛",@"des":@"巴比特始建于2011年,是国内最早的区块链(blockchain)咨询社区门户"},@{@"icon":@"jinsecaijing",@"name":@"金色财经",@"des":@"金色财经是集行业新闻,资讯,行情,数据等一站式区块产业服务平台"},@{@"icon":@"LSK",@"name":@"LSK",@"des":@"Lsk是基于自己的区块链网络和代币LSK"},@{@"icon":@"ccm-1",@"name":@"SRA数字资产交易所",@"des":@"SRA数字资产交易所于2019年8月23日正式开始交易,众多资产持有人可进行入场交易"}, nil];
    }
    return _dataArray;
}
