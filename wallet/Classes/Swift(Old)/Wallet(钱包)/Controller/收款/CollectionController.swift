...
class CollectionController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        queue.addOperation {
                DispatchQueue.main.async {
                    ...
                    self.moneyLB.text = "金额" + String(format:"%f",self.token!.balance) + self.token!.name
                }
            }
        }
    ...
    @IBAction func copyAction(_ sender: UIButton) {
        ...
        window?.makeToast(UIImage.init(named: "ico_assets_copyed")!, bottomMSG: "地址复制成功")
    }
    //保存图片
    ...
    @IBAction func sureMoney(_ sender: Any) {
        self.moneyLB.text = "金额" + self.moneyTF.text! + self.token!.name
    }
    /*
    // MARK: - Navigation
