...
class TransferController: BaseViewController,UITextFieldDelegate,QRReaderVCDelegate {
    ...
    override func viewDidLoad() {
        ...
        adressTF.attributedPlaceholder = NSAttributedString(string: "请输入账户地址",attributes: placeholserAttributes)
        moneyTF.attributedPlaceholder = NSAttributedString(string: "请输入金额",attributes: placeholserAttributes)
        
        
        self.BalanceLabel.text = String(format: "余额: %f \(self.tokenModel!.name)", self.tokenModel!.balance)

        self.moneyTF.rac_textSignal().subscribeNext { (money) in
            
            self.BalanceLabel.text = String(format: "余额: %f \(self.tokenModel!.name)", self.tokenModel!.balance - money!.doubleValue)
        }
...
//                 self.itemModel?.value = gasprice
                ...
                if (!txHash.hex().hasPrefix("0x")) {
                    self.view.makeToast("转账失败", duration: 0.3, position: .center)
                    return
                }
...
//                self.navigationController?.cw_push(VC)
                ...
                self.view.makeToast("余额不足", duration: 0.3, position: .center)
                print(error)
        }
    }
    //BTC 转账
                    ...
                    case true:
                        ...
                        do {
                            ...
                            try  BTCWallet.send(to: btcAddress, amount: UInt64(number), fee: UInt64(fee), utxos: self.utxos, completion: { (json) in
                                if json == "Error" {
                                    self.view.makeToast("转账失败", duration: 1.0, position: .center)
                                }
                                else {
                                    self.view.makeToast("转账成功", duration: 1.0, position: .center)
...
//                self.itemModel?.value = gasprice
    ...
    @IBAction func sureTransfer(_ sender: UIButton) {
        guard adressTF.text != "" else {
            self.view.makeToast("请输入账户", duration: 0.3, position: .center)
            return
        }
        guard moneyTF.text != "" else {
            self.view.makeToast("请输入转账金额", duration: 0.3, position: .center)
            return
        }
        ...
        CustomUserManager.customShared().showWalletPassWordViewFinish { (input_Text) in
            if (password == input_Text) {
                ...
                self.view.makeToast("密码输入错误", duration: 0.3, position: .center)
            }
        }
...
//        self.itemModel!.input = String((json?.transaction.inferedChainID)!.rawValue)
        MBProgressHUD .showMessage("转账成功");
